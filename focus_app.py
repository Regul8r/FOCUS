...
```
DATA_FILE = os.path.join(os.path.dirname(__file__), "focus_data.json")
...
<style>
    ...
    .face-green {
        background: linear-gradient(160deg, #0a2e18, #0f3d22);
        border-color: #2ecc71;
        box-shadow: 0 0 24px rgba(46,204,113,0.2), inset 0 1px 0 rgba(46,204,113,0.1);
    }
    .face-yellow {
        background: linear-gradient(160deg, #2e2000, #3d2c00);
        border-color: #f1c40f;
        box-shadow: 0 0 24px rgba(241,196,15,0.2), inset 0 1px 0 rgba(241,196,15,0.1);
    }
    .face-red {
        background: linear-gradient(160deg, #2e0000, #3d0a0a);
        border-color: #e74c3c;
        box-shadow: 0 0 24px rgba(231,76,60,0.25), inset 0 1px 0 rgba(231,76,60,0.1);
    ...
    @keyframes redpulse {
        0%, 100% { box-shadow: 0 0 24px rgba(231,76,60,0.25); }
    ...
    .face-category { font-size: 0.68rem; opacity: 0.55; text-transform: uppercase; letter-spacing: 0.1em; }
    ...
    .prog-fill-green  { height: 5px; border-radius: 99px; background: #2ecc71; transition: width 0.6s ease; }
    .prog-fill-yellow { height: 5px; border-radius: 99px; background: #f1c40f; transition: width 0.6s ease; }
    .prog-fill-red    { height: 5px; border-radius: 99px; background: #e74c3c; transition: width 0.6s ease; }
...
def get_nudge(account):
    ...
    msg = ai.messages.create(
        ...
        messages=[{"role": "user", "content":
            f"You are FOCUS, a financial awareness assistant for college students. "
            f"Account: {account['name']} ({account['category']}), "
            f"Status: {health_context}, "
            f"Type: {'credit card — lower is better' if account['type'] == 'credit' else 'savings — higher is better'}. "
...
def progress_bar(account):
    ...
    goal_label = f"Goal: ${goal:,.0f}"
    return f"""
    <div class="prog-track">
        <div class="prog-fill-{health}" style="width:{pct:.0f}%"></div>
    </div>
    <div class="face-goal">{goal_label} &nbsp;·&nbsp; {pct:.0f}% there</div>
    """
...
def render_face(account, col):
    ...
    health    = account["health"]
    css       = "face-dismissed" if dismissed else f"face-{health}"
    ...
    with col:
        ...
        if st.button("✦ Get Insight", key=f"n{aid}", use_container_width=True):
            with st.spinner("Claude is analyzing..."):
                try:    st.session_state.nudges[aid] = get_nudge(account)
                except Exception as e: st.session_state.nudges[aid] = f"Error: {e}"
            st.rerun()
...
if accounts:
    ...
    freshness = f"Updated {int(age)}s ago" if age < 30 else "Reconnecting..."
